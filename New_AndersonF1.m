function [YY,Z] = New_AndersonF1(ss,n,c,influence,P,trials)
...
%                   100 by default)
%   P:              chooses the particular delta vector (options are 1, 2,
%                   and 3, with 3 being the "widest"
...
if nargin <=4
    P = 3; % Chooses the "widest" vector by default
end
...
parfor iii = 1:trials
    ...
    for ll = 1:max
        ...
        if s == 1
            ...
            s1 = 0.999999999999; % Helps with averaging
            s2 = 1.000000000001; % Helps with averaging
            
            for mm = 1:2
                
                % Chooses s-value for averaging
                if mm == 1
                    s = s1;
                elseif mm == 2
                    s = s2;
                ...
                W = zeros(n-2,2); % Temporary storage for the averaging
    
                A = zeros(N,3); % Placeholder vector for simulation
                A(NN,1) = 1; % Builds delta_0
                ...
                for i=2:(n-1)
                    ...
                    W(i-1,mm) = sqrt(1-p); % Stores computed distance
                ...
                Y(:,ll) = Y(:,ll) + W(:,mm); % Aids in averaging
    
            end     
        
            Y(:,ll) = 0.5*Y(:,ll); % Computes the average of the runs
...
data = ['Frac_Data_s_' num2str(s(1)) '-' num2str(s(max)) '_c_' cString '_n_' num2str(n) '_infl_' num2str(influence) '_trials_' num2str(trials) '.mat'];
